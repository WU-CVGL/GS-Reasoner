...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    if not is_multimodal:
        return sources
    # put the DEFAULT_IMAGE_TOKEN in the begin of the sentence
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    text_question_mask_idx = [nl_tokens[0], im_start, im_end, image_token_index, tokenizer.convert_tokens_to_ids("user")]
    ...
    for i, source in enumerate(sources):
        ...
        text_question_mask += [False] * len(input_id)
        ...
        for conv in source:
            ...
            if content is not None:            
                ...
                # used for test
                encode_id = tokenizer('<|im_start|>'+role+'\n').input_ids 
                
            input_id += encode_id
            if role in ["user", "system"]:
                target += [IGNORE_INDEX] * len(encode_id)
                text_question_mask += [True] * len(encode_id)
            else:
                target += encode_id
                text_question_mask += [False] * len(encode_id)
        
                    
        assert len(input_id) == len(target), f"{len(input_id)} != {len(target)}"
        assert len(input_id) == len(text_question_mask), f"{len(input_id)} != {len(text_question_mask)}"
        for idx, encode_id in enumerate(input_id):
            if encode_id in unmask_tokens_idx:
                target[idx] = encode_id
            if encode_id == image_token_index:
                input_id[idx] = IMAGE_TOKEN_INDEX
            if encode_id in text_question_mask_idx:
                text_question_mask[idx] = False
        input_ids.append(input_id)
        targets.append(target)
    
    #! do not consider different text length 
...
def get_positive_mask(input_tokens, positive_tokens):
    token_indexes = []
    len_qt = len(input_tokens)
    len_gt = len(positive_tokens)
    for i in range(len_qt - len_gt + 1):
        if torch.all(input_tokens[i:i+len_gt] == positive_tokens):
            token_indexes += [j for j in range(i, i + len_gt)]
    if len(token_indexes) == 0:
        token_indexes = [i for i, x in enumerate(input_tokens) if torch.all(positive_tokens[-1] == x)]

    positive_mask = torch.zeros_like(input_tokens, dtype=torch.bool)
    positive_mask[token_indexes] = True
