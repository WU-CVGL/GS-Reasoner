...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        if self.mode in ["train", "test"]:  # train mode
            if self.mode == "train":
                # random select one point in each voxel
                idx_select = (
                    np.cumsum(np.insert(count, 0, 0)[0:-1])
                    + np.random.randint(0, count.max(), count.size) % count
                )
            else:
                # select the middel point in every voxel
                idx_select = (
