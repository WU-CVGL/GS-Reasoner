...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, BBOX_START_TOKEN, BBOX_END_TOKEN, THINK_START_TOKEN, THINK_END_TOKEN, ANSWER_START_TOKEN, ANSWER_END_TOKEN
...
import math
from llava.utils_3d import transform_point, convert_bbox_to_str
...
def eval_model(questions, args):
    ...
    for line in tqdm(questions):
        ...
        if line["metadata"].get("object", None) is not None:
            # process box input
            obj_bbox_lst = torch.tensor(line["metadata"]["object"][0]["object_bbox"])
            ...
            obj_bbox_str = f"{BBOX_START_TOKEN}{obj_bbox_str_lst[0]}{BBOX_END_TOKEN}"
            cur_prompt = cur_prompt.replace("<ground>", obj_bbox_str)
        ...
        outputs = outputs.split(ANSWER_START_TOKEN)[-1].replace(ANSWER_END_TOKEN, "")
