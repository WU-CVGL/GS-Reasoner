...
from llava.utils_3d import aabb_to_xyzwlh, decode_str_to_bbox, find_best_bbox_by_iou
...
def build_frame_path(scene_id, frame_ids):
    base_path = "data/processed_data/scannetpp/color/val"
    return [os.path.join(base_path, f"{scene_id}/frame_{i:06d}.jpg") for i in frame_ids]
...
def eval_model(questions, args):
    ...
    for line in tqdm(questions):
        ...
        if line["metadata"].get("frames_used", None) is not None:
            frame_used = line["metadata"]["frames_used"]
            frame_path = build_frame_path(video_id.split("/")[1], frame_used)
        else:
            frame_path = None
        ...
        if line["metadata"].get("target_object_id", None) is not None:
            target_object_id = line["metadata"].get("target_object_id", None)

            for obj_info in line["metadata"]["object"]:
                obj_ids = obj_info["object_id"]
                obj_bboxes = obj_info["object_bbox"]
                if target_object_id in obj_ids:
                    gt_response = obj_bboxes[obj_ids.index(target_object_id)]
                    break
            
            all_pred_bbox = [bbox["object_bbox"][0] for bbox in bbox_lst]
            all_pred_bbox = [aabb_to_xyzwlh(bbox) for bbox in all_pred_bbox]
            # Find the bbox with highest IoU with ground truth
            pred_response, _ = find_best_bbox_by_iou(all_pred_bbox, gt_response)
        else:
            pred_bbox = bbox_lst[0]["object_bbox"][0]
            gt_response = line["metadata"]["object"][0]["object_bbox"][0]
            all_pred_bbox = [bbox["object_bbox"][0] for bbox in bbox_lst]
            pred_response = aabb_to_xyzwlh(pred_bbox)      
        
        object_name = bbox_lst[0]["object_name"]
